...
public class Canvas extends JPanel implements ActionListener {
	 ...
	 * @param units
	 *            The units to be shown on the grid
	 */
	...
	private void init() {
		/*
		 * Creates the first popup menu New line;Close curve; Open curve
		 */
		...
		/*
		 * Creates the second popup menu Move point; delete point
		 */
		...
		/*
		 * Add a mouse listener to the canvas Add new point; Open popup 1; Open
		 * popup 2; Some logic for dragging points; Enter coordiantes
		 */
		addMouseListener(new MouseListener() {
			...
			public void mousePressed(MouseEvent e) {
				if (SwingUtilities.isLeftMouseButton(e) && e.isControlDown()) {
					if (curveID != -1) {
						fireEvent(new GuiEventsAdd(this, new double[] {
								xm(e.getX()), ym(e.getY()) }, curveID));
					}
					...
					for (int i = 0; i < controlPoints.size(); i++) {
						if (controlPoints.get(i).contains(e.getX(), e.getY())) {
							...
							if (SwingUtilities.isRightMouseButton(e)) {
								popup2.show(e.getComponent(), e.getX(),
										e.getY());
							}
						}
					}
		...
		/*
		 * Add the motion mouse listener Move point
		 */
		addMouseMotionListener(new MouseMotionListener() {
			...
			public void mouseDragged(MouseEvent e) {
				if (!first) {
					...
					fireEvent(new GuiEventsMove(this, new double[] {
							xm(e.getX()), ym(e.getY()) }, selectPoint, curveID));
				}
		...
		addMouseWheelListener((MouseWheelEvent e) -> {
			if (Math.signum(e.getPreciseWheelRotation()) == 1) {
				setZoom(Math.abs(getZoom()
						/ (e.getPreciseWheelRotation() * 1.05)));
			} else {
				setZoom(Math
						.abs(e.getPreciseWheelRotation() * 1.05 * getZoom()));
			}
		});
	...
	private void openNewPopUpCoordiante() {
		String tmp = JOptionPane.showInputDialog(this, "Enter Coordiante",
				"0.0, 0.0");
		try {
			tmp = tmp.replace(" ", "");
	...
	private void openNewPopUpCoordianteMove() {
		...
		try {
			...
			fireEvent(new GuiEventsMove(this, new double[] { x, y },
					selectPoint, curveID));
		} catch (Exception e) {
			System.out.println("Please enter a proper location " + e);
		}
	...
	private void drawGrid(Graphics2D g) {
		for (int i = -500; i < 500; i++) {
			...
			g.drawLine(Integer.MAX_VALUE * -1, (int) y(gridspacing * i),
					Integer.MAX_VALUE, (int) y(gridspacing * i));
			g.drawLine((int) x(gridspacing * i), Integer.MAX_VALUE * -1,
					(int) x(gridspacing * i), Integer.MAX_VALUE);
			g.drawString(Double.toString(i * units), (int) x(i * gridspacing
					+ 5), (int) y(0) - 2);
			g.drawString(Double.toString(i * units), (int) x(5), (int) y(i
					* gridspacing) - 2);
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().getClass().isInstance(new JMenuItem())) {
			...
			if (tmp.getParent().equals(popup1)) {
				...
				/*
				 * Does the action if the user clicks on "New Line" or
				 * "Close curve" or "Open curve"
				 */
				if (e.getActionCommand().equals("New Line")) {
					String optionSelected = (String) JOptionPane
							.showInputDialog(this, "Please Select a Line Type",
									"Line Type", JOptionPane.QUESTION_MESSAGE,
									null, new String[] { "PolyLine",
											"Cubic Line Natural",
											"Bezier Curve", "Bezier Spline",
											"Bezier Spline Colinear" }, null);
					/*
					 * Fires the currect event based on what was selected
					 */
					if (optionSelected != null) {
						String name = (String) JOptionPane.showInputDialog(
								this, "Please Give the Line a name",
								"Line Name", JOptionPane.QUESTION_MESSAGE);
						if (name != null) {
							curveID++;
							int type = 0;
							switch (optionSelected) {
							case "PolyLine":
								type = Controller.POLYLINE;
								break;
							case "Cubic Line Natural":
								type = Controller.CUBIC_N;
								break;
							case "Bezier Curve":
								type = Controller.BEZIERCURVE;
								break;
							case "Bezier Spline":
								type = Controller.BEZIERSPLINE;
								break;
							case "Bezier Spline Colinear":
								type = Controller.BEZIERSPLINECOLINEAR;
								break;
							default:
							...
							fireEvent(new GuiEventsCreate(this, new double[] {
									point.x, point.y, type }, name));
						}
					}
	 ...
	 *
	 * @param y
	 *            The y location the object
	 * @return The corrected y location
	 */
	private double y(double y) {
		return (zoom * getVisibleRect().height / 2 - zoom * y)
				- ((zoom - 1) * getVisibleRect().height / 2) - offSetY;
	}
	...
	private void updateControls() {
		...
		for (Point2D control : controls) {
			controlPoints.add(new Ellipse2D.Double(
					x(control.getX()) - size / 2, y(control.getY()) - size / 2,
					size, size));
		}
	}
	 ...
	 *
	 * @param visiblity
	 *            Boolean to set if its true or false
	 */
